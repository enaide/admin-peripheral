...
export class PeripheralComponent implements OnInit
{
    ...
    displayedColumns: string[] = ['serial', 'name', 'ipv4_addressd', 'count', 'Ficha'];
    ...
    init_table(){
        ...
        .pipe(
            ...
            if(+res.code === 200){
                this.gateways = res.body.map(r => <any>{...r, count: r.devices.items.length});
    ...
    removeGateway(periphId: string){
        ...
        this.gatewayService.remove(periphId)
        .pipe(
            catchError(err => {        
                this.isLoadingResults = false;
                return of({error: err});
